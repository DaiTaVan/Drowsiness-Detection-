...
report_output_path = 'output/report'
...
class participent:
    def __init__(self, participent,mood, location):
    ...
    def process_one_mood(self, width, progress_report):
        # Reporting results
        dropped_frames = []

        # Metadata
        video_rotation, video_fps, size_x, size_y = self.get_video_metadata(self.location)
        ...
        fvs = FileVideoStream(self.location).start()
        ...
        result_df.to_csv(os.path.join(csv_output_path, f"{self.participant_no}_{self.mood}.csv"))

        with open(os.path.join(report_output_path, f"{self.participant_no}_{self.mood}.txt"), "w") as report:
            report.write(f"video: {self.location}\n")
...
vid_process = participent(14,10,'Data_raw/14/10.mp4')
vid_process.process_one_mood(width,progress_report)
# %% Main
